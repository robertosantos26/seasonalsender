...
## 4) Currículo, assunto e mensagem por categoria

Na aba **Configurações**, cada categoria de vaga (Farm, Horse, Housekeeper,
Server, Landscaping, Geral...) tem seu próprio bloco com:

- **Palavras-chave** — usadas para classificar a vaga automaticamente.
- **Assunto** — o assunto do e-mail para essa categoria (deixe em branco para
  usar o assunto padrão geral).
- **Mensagem** — o corpo do e-mail para essa categoria (deixe em branco para
  usar a mensagem padrão geral). Use `{job_title}`, `{company}`, `{your_name}`
  e `{your_phone}` como variáveis.
- **Currículo PDF** e **Cover letter PDF (opcional)** — envie o arquivo direto
  pela página, pelo botão de upload. O PDF é salvo no banco de dados
  (Supabase), então não precisa subir arquivo nenhum para o GitHub nem mexer
  no repositório — só escolher o arquivo e ele já fica valendo no próximo
  envio.

Assim, cada tipo de vaga sai com currículo e mensagem diferentes: por exemplo,
uma vaga de housekeeper recebe o currículo de housekeeper com a mensagem de
housekeeper, e uma vaga de construção recebe o currículo e a mensagem
cadastrados na categoria correspondente. Depois de subir os PDFs e ajustar os
textos, clique em **Salvar configurações**.

> Observação: os PDFs de exemplo que já estão na pasta `/curriculos/` do
> repositório continuam funcionando como currículo padrão (fallback) enquanto
> você não enviar um novo pelo upload; depois de enviar pela página, o arquivo
> do banco passa a ter prioridade.
