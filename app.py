...
def attach_file(msg, folder, filename):
    """Anexa um arquivo ao email se existir."""
...
def send_email_smtp(to_email, subject, body, cv_file, config, cover_letter_file=""):
    try:
        ...
        # Anexa currículo se fornecido
        if cv_file:
            attach_file(msg, "curriculos", cv_file)
        # Anexa cover letter se configurada
        if cover_letter_file:
            attach_file(msg, "curriculos", cover_letter_file)
        port = int(config["smtp_port"])
        # Porta 465 usa SSL direto, porta 587 usa STARTTLS
        if port == 465:
            srv = smtplib.SMTP_SSL(config["smtp_server"], port, timeout=15)
        else:
            srv = smtplib.SMTP(config["smtp_server"], port, timeout=15)
            srv.ehlo()
            srv.starttls()
            srv.ehlo()
        ...
        return False, "Erro de autenticação — verifique email e senha App"
    except smtplib.SMTPRecipientsRefused:
        return False, f"Email recusado pelo servidor: {to_email}"
    except Exception as e:
        return False, str(e)
