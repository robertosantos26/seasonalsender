...
def send_email_smtp(to_email, subject, body, cv_file, config, cover_letter_file=""):
    """Envia email via Resend API — funciona no Render gratuito."""
    resend_key   = os.environ.get("RESEND_API_KEY", "")
    sender_email = config.get("email", "")
    sender_name  = config.get("name", "SeasonalSender")

    if not resend_key:
        return False, "RESEND_API_KEY não configurada no Render → Environment."
    if not sender_email:
        return False, "Email do remetente não configurado em Configurações."

    # Sem domínio verificado, usa o domínio do Resend como from
    # e o Gmail do usuário como reply_to — recrutador responde pro Gmail
    resend_from = f"{sender_name} <onboarding@resend.dev>"

    payload = {
        "from":     resend_from,
        "to":       [to_email],
        "bcc":      [sender_email],   # cópia para seu Gmail
        "reply_to": sender_email,     # respostas vão pro seu Gmail
        "subject":  subject,
        "text":     body,
    }
    ...
    try:
        ...
        if resp.status_code in (200, 201):
            return True, "Enviado"
        msg = data.get("message") or data.get("name") or f"Erro {resp.status_code}"
        return False, msg
    except Exception as e:
        return False, str(e)
...
def api_status():
    ...
    resend_key = os.environ.get("RESEND_API_KEY","")
    if not resend_key:
        status["email"] = "❌ RESEND_API_KEY não configurada"
    else:
        status["email"] = "✅ Resend configurado"
