...
def api_status():
    ...
    if not mongo_uri:
        return jsonify({
            "mongo": False,
            "step": "MONGO_URI não encontrada",
            "fix": "Adicione a variável MONGO_URI no painel Environment do Render",
            "uri_set": False
        })
    try:
        db = get_db()
        db.command("ping")
        jobs_count = col_jobs().count_documents({})
        return jsonify({
            "mongo": True,
            "step": "Conectado com sucesso!",
            "jobs_no_banco": jobs_count,
            "uri_preview": mongo_uri[:30] + "..."
        })
    except Exception as e:
        return jsonify({
            "mongo": False,
            "step": "URI encontrada mas conexão falhou",
            "erro": str(e),
            "uri_preview": mongo_uri[:30] + "...",
            "fix": "Verifique se a senha está correta e se o IP 0.0.0.0/0 está liberado no MongoDB Atlas"
        })
...
def api_scrape():
    try:
        ...
        c = col_jobs()
        total_in_db = c.count_documents({}) if c else 0
        return jsonify({"success": True, "scraped": len(jobs), "new": added, "total": total_in_db})
    except Exception as e:
        return jsonify({"success": False, "error": str(e), "scraped": 0, "new": 0})

@app.route('/api/enrich/<job_id>', methods=['POST'])
def api_enrich(job_id):
    """Carrega email/telefone/salário da página da vaga (chamado manualmente)."""
    try:
        c = col_jobs()
        ...
        if not job.get('url'):
            return jsonify({"success": False, "message": "Vaga sem URL"})

        resp = requests.get(job['url'], headers={"User-Agent": "Mozilla/5.0"}, timeout=12)
        ...
        if detail and c:
            c.update_one({"id": job_id}, {"$set": detail})
            job.update(detail)
