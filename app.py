...
def send_email_smtp(to_email, subject, body, cv_file, config, cover_letter_file=""):
    """Envia email via SMTP — funciona no Railway."""
    sender_email = config.get("email", "")
    sender_pass  = config.get("email_password", "")
    sender_name  = config.get("name", "SeasonalSender")

    if not sender_email or not sender_pass:
        return False, "Email ou senha não configurados em Configurações."

    try:
        msg = MIMEMultipart()
        msg["From"]    = f"{sender_name} <{sender_email}>"
        msg["To"]      = to_email
        msg["Bcc"]     = sender_email  # cópia automática para você
        msg["Reply-To"]= sender_email
        msg["Subject"] = subject
        msg.attach(MIMEText(body, "plain"))

        # Anexa currículo e cover letter se existirem
        ...
        if port == 465:
            srv = smtplib.SMTP_SSL(server, port, timeout=20)
        else:
            srv = smtplib.SMTP(server, port, timeout=20)
            srv.ehlo()
            srv.starttls()
            srv.ehlo()

        srv.login(sender_email, sender_pass)
        srv.sendmail(sender_email, [to_email, sender_email], msg.as_string())
        srv.quit()
        return True, "Enviado"

    except smtplib.SMTPAuthenticationError:
        return False, "Erro de autenticação — verifique a Senha de App do Gmail"
    except smtplib.SMTPRecipientsRefused:
        return False, f"Email do destinatário recusado: {to_email}"
    except Exception as e:
        return False, str(e)
