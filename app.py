...
# ── DOL Official JSON Feeds ──────────────────────────────
# Feeds oficiais: últimos 20 dias, atualizados diariamente à meia-noite EST
# jo = H-2A Job Orders | h2a = H-2A applications | h2b = H-2B applications
FEED_BASE = "https://api.seasonaljobs.dol.gov/datahub-search/sjCaseData/zip"

def download_zip_feed(feed_type, date_str):
    """Baixa ZIP do feed oficial e retorna lista de registros JSON."""
    url = f"{FEED_BASE}/{feed_type}/{date_str}"
    try:
        import zipfile, io
...
def parse_record(record, agri, date_str):
    """Converte registro do feed em formato padrão da plataforma."""
    try:
        # Campos comuns H-2A e H-2B
        title    = (record.get("JOB_TITLE") or record.get("OCCUPATION_TITLE") or
                    record.get("SECOND_OCCUPATION_TITLE") or "Seasonal Worker").strip()
        company  = (record.get("EMPLOYER_NAME") or record.get("TRADE_NAME_DBA") or "").strip()
        city     = (record.get("EMPLOYER_CITY") or record.get("WORKSITE_CITY") or "").strip()
        state    = (record.get("EMPLOYER_STATE") or record.get("WORKSITE_STATE") or "").strip()
        location = f"{city}, {state}".strip(", ")
        email    = (record.get("EMPLOYER_POC_EMAIL") or record.get("EMPLOYER_EMAIL") or
                    record.get("AGENT_ATTORNEY_EMAIL") or "").strip().lower()
        phone    = (record.get("EMPLOYER_POC_PHONE") or record.get("EMPLOYER_PHONE") or "").strip()
        wage_from = record.get("WAGE_RATE_OF_PAY_FROM") or record.get("BASIC_RATE_OF_PAY") or ""
        wage_unit = record.get("WAGE_UNIT_OF_PAY") or "hour"
        salary   = f"${wage_from}/{wage_unit}" if wage_from else ""
        case_num = (record.get("CASE_NUMBER") or record.get("CASE_NO") or "").strip()
        desc_parts = [
            record.get("JOB_DESCRIPTION") or "",
            record.get("SPECIFIC_REQUIREMENTS") or "",
            record.get("ADDITIONAL_WAGE_INFO") or "",
        ]
        description = " ".join(p for p in desc_parts if p)[:600]
        url = f"https://seasonaljobs.dol.gov/jobs/{case_num}" if case_num else ""

        # Data do caso
        raw_date = (record.get("CASE_RECEIVED_DATE") or
                    record.get("EMPLOYMENT_BEGIN_DATE") or
                    record.get("BEGIN_DATE") or date_str)
        try:
        ...
        if not case_num and not title:
            return None

        job_id = f"dol_{case_num}" if case_num else f"dol_{abs(hash(title+company+location))}"
...
def fetch_rss_jobs(keywords="", job_type="all"):
    """Baixa os 3 feeds oficiais do DOL para a data de hoje."""
    ...
    if job_type in ("all", "agricultural"):
        feeds_to_fetch.append(("jo",  True,  "H-2A Job Orders"))
        feeds_to_fetch.append(("h2a", True,  "H-2A Applications"))
    if job_type in ("all", "non-agricultural"):
        feeds_to_fetch.append(("h2b", False, "H-2B Applications"))

    for feed_type, agri, label in feeds_to_fetch:
        records = download_zip_feed(feed_type, today)
        count = 0
        for r in records:
            job = parse_record(r, agri, today)
            if not job:
                continue
            if kw_list:
            ...
            jobs.append(job)
            count += 1
        add_log(f"Feed {label}: {count} vagas carregadas.", "found")

    # Remove duplicatas por ID (jo e h2a podem ter sobreposição)
    seen = set()
    unique = []
    for j in jobs:
        if j["id"] not in seen:
    ...
    add_log(f"Total: {len(unique)} vagas únicas dos feeds oficiais DOL.", "found")
    return unique, None
...
def api_status():
    if not DATABASE_URL:
        return jsonify({
            "ok": False,
            "mensagem": "DATABASE_URL não configurada no Render",
            "fix": "Veja as instruções abaixo"
        })
    try:
        init_db()
        ...
        return jsonify({
            ...
            "vagas_no_banco": row["total"] if row else 0
        })
    except Exception as e:
        return jsonify({
            "ok": False,
            "mensagem": str(e),
            "fix": "Verifique se DATABASE_URL está correta e tem ?sslmode=require"
        })
