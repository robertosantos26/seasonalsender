...
def safe_display_name(name):
    """Transforma o 'Nome completo' das Configurações num prefixo seguro para nome
    de arquivo, ex: 'Roberto Santos' -> 'Roberto_Santos_'. Vazio se não configurado."""
    name = (name or "").strip()
    if not name:
        return ""
...
def display_attachment_name(stored_filename, config):
    """Fallback para anexos antigos que ainda não têm o nome embutido no arquivo
    salvo (uploads feitos antes desta correção) — prefixa o nome na hora do envio
    sem duplicar caso o arquivo já tenha sido salvo com o nome (uploads novos)."""
    if not stored_filename:
        return stored_filename
    prefix = safe_display_name(config.get("name"))
    if not prefix or stored_filename.startswith(prefix):
        return stored_filename
...
def track_pixel(tid):
    try:
        ...
        if row:
            ...
            first_open = not job.get("opened")
            job["opened"] = True
            job["open_count"] = int(job.get("open_count") or 0) + 1
            if first_open:
                job["opened_at"] = now
            job["last_opened_at"] = now
            ...
            if first_open:
                ...
                add_log(f"👁 Email reaberto ({job['open_count']}x): {job.get('title','')} — {job.get('contactEmail','')}", "opened")
    except:
        pass
...
def api_upload_curriculo():
    ...
    try:
        ...
        fname = f"{safe_display_name(config.get('name'))}{cat_id}_{kind}.pdf"
...
@app.route("/api/jobs/<job_id>/category", methods=["POST"])
def api_update_job_category(job_id):
    """Permite corrigir manualmente o setor/categoria de uma vaga (ex: farm -> housekeeping).
    O próximo email enviado para essa vaga usará o currículo/carta/template da nova categoria."""
    try:
        body = request.get_json(silent=True) or {}
        new_cat_id = (body.get("category") or "").strip()
        if not new_cat_id:
        ...
        if new_cat_id not in valid_ids:
            return jsonify({"success": False, "message": f"Categoria inválida: {new_cat_id}"})
        ...
        job = row["data"]
        old_cat_id = job.get("category", "general")
        job["category"] = new_cat_id
        job["category_manual"] = True  # marca que foi corrigida à mão, não sobrescrever automaticamente depois
        db_query("UPDATE jobs SET data = %s WHERE id = %s", (json.dumps(job), job_id))

        old_label = get_category(config, old_cat_id).get("label", old_cat_id)
        new_label = get_category(config, new_cat_id).get("label", new_cat_id)
        add_log(f"✎ Setor alterado: {job.get('title','')} — {old_label} → {new_label}", "info")

        return jsonify({"success": True, "category": new_cat_id})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)})
        ...
        if config.get("email_password") or os.environ.get("GAS_WEBHOOK_URL"):
            ...
            if success:
                job.update({"status":"sent","sent_date":datetime.now().strftime("%d/%m/%Y"),
                            ...
                            "open_count":0,"opened_at":None,"last_opened_at":None})
                db_query("UPDATE jobs SET data = %s, status = 'sent' WHERE id = %s",
                         (json.dumps(job), job_id))
    ...
    try:
        ...
        row = db_query("""
            SELECT
                ...
                COALESCE(SUM((data->>'open_count')::int), 0) as total_opens,
        ...
        return jsonify(dict(row)) if row else jsonify({
            "total":0,"sent":0,"pending":0,"today":0,"sent_today":0,"opened":0,"total_opens":0,"with_email":0})
    except Exception as e:
        return jsonify({"total":0,"sent":0,"pending":0,"today":0,"sent_today":0,"opened":0,"total_opens":0,"with_email":0})
    ...
    try:
        ...
        for r in (rows or []):
            ...
            out.append({
                ...
                "open_count": int(j.get("open_count") or 0), "last_opened_at": j.get("last_opened_at"),
                "status": j.get("status"),
            })
        return jsonify(out)
    except Exception as e:
