...
def get_pool():
    ...
    if _pool is None and DATABASE_URL:
        url = DATABASE_URL
        # Supabase free tier só aceita IPv4 — força via host resolvido
        # Troca host padrão pelo pooler IPv4 do Supabase
        url = url.replace(
            "db.amkzekunvrpendbofxel.supabase.co",
            "aws-0-us-east-1.pooler.supabase.com"
        )
        # Ajusta usuário para formato do pooler (postgres.PROJECT_ID)
        url = url.replace(
            "postgresql://postgres:",
            "postgresql://postgres.amkzekunvrpendbofxel:"
        )
        # Troca porta para 6543 (pooler) se ainda estiver em 5432
