...
# ── Worker da fila de envio ──────────────────────────────
def queue_worker():
    """Processa a fila enviando 1 email a cada 5 minutos."""
    ...
    while not _send_queue.empty():
        try:
            ...
            # Aguarda 5 minutos antes do próximo — só se ainda tiver emails na fila
            if not _send_queue.empty():
                _queue_status["current"] = f"Aguardando 5 min... ({_send_queue.qsize()} restantes)"
...
def api_send_all():
    global _queue_running, _queue_status
    try:
        if _queue_running:
            return jsonify({
                "success": False,
        ...
        # Busca todos os pendentes com email
        rows = db_query("""
        ...
        # Inicia worker em thread separada (não bloqueia o servidor)
        t = threading.Thread(target=queue_worker, daemon=True)
        t.start()

        return jsonify({
            "success": True,
            "message": f"Fila iniciada! {len(pending)} emails serão enviados 1 a cada 5 minutos.",
            "total": len(pending),
            "estimated_minutes": len(pending) * 5
        })
    except Exception as e:
...
def api_queue_status():
    """Retorna status atual da fila de envio."""
    return jsonify({
        **_queue_status,
