...
def get_pool():
    ...
    if _pool is None and DATABASE_URL:
        # Monta URI do pooler IPv4 do Supabase direto
        # Extrai só a senha da DATABASE_URL original
        import re
        m = re.search(r'postgresql://[^:]+:([^@]+)@', DATABASE_URL)
        senha = m.group(1) if m else ""
        project_id = "amkzekunvrpendbofxel"
        url = (
            f"postgresql://postgres.{project_id}:{senha}"
            f"@aws-1-us-west-2.pooler.supabase.com:6543/postgres"
            f"?sslmode=require"
        )
